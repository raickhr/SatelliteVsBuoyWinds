...
def printFig(lat, lon):
    ...
    returnText = f'2007 to 2014 | {LAT:3d} | {LON:4d} | '
    matchFileName = f'../../downloads/Buoy/extractedGZ2/WINDS/T_{lat:02d}{latUnits}_{lon:03d}{lonUnits}_matchupNearestFour_2007.nc'
    #matchFileName = f'../../downloads/SusanDownload/TMA_HR/MET/T_{lat:02d}{latUnits}_{lon:03d}{lonUnits}_matchupNearestFour_2007.nc'
    ...
    if not os.path.isfile(matchFileName):
        ...
        wspdS = np.array(ds.variables['U10N_AS'][0,:])
        wdirS = np.array(ds.variables['U10N_dir_AS'][0,:])
        wdirS = (450-wdirS)%360
        ...
        ax = axes[0]
        ax.plot(x, wspdS, alpha = 0.5, label = 'AS')
        ...
        returnText += f'| {mean:.2f} | {std:.2f} |{r:.2f}'

        ax.plot(x, diff, label = 'AS-buoy')
        ax.legend()
        ax.text(0.5, 0.9, text, horizontalalignment='center',
            verticalalignment='center', transform=ax.transAxes)
        ax.grid(visible=True, which='both', axis='both',alpha = 0.5)
        ...
        ax = axes[2]
        ax.plot(x, wdirS, alpha= 0.5, label = 'AS')
        ...
        ###################################################################
        text = 'Spatial Distance between AS data and buoy Pos'
        ...
        ###################################################################
        text = 'Difference in time between AS measuremnt and buoy measurement'
        ...
        text = f'{lat:d}{latUnits} {lon:d}{lonUnits} 10min-matchup buoy Neutral Winds with AS data 2007-2014'
        fig.suptitle(text, y = 0.9)
        plt.subplots_adjust(hspace=0.5)

        fname = f'images/ASCATcomp/{lat:02d}{latUnits}_{lon:03d}{lonUnits}_Comp_2007to2014_ASCAT_afterErrRemoved.png'
