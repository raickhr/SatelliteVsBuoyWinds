...
for task in taskList:
    ...
    if os.path.isfile(matchFname):
        try:
            ...
            ax1, ax2 = axes[c_label,0], axes[c_label,1]
            textA, textB = plotTimeSeriesByClusterLabel(df, LAT, LON, ax1, ax2, cluster_label=c_label, color = 'blue')
            ax1.text(0.1,0.85, textA,  horizontalalignment='left',
                        verticalalignment='center', transform=ax1.transAxes)
            ax1.set_ylabel(f'label {c_label:d} speed diff.')
            ax1.set_ylim(-5,5)

            ax2.text(0.1,0.85, textB,  horizontalalignment='left',
                        verticalalignment='center', transform=ax2.transAxes)
            ax2.set_ylabel(f'label {c_label:d} direction diff.')
    ...
    if os.path.isfile(matchFname):
        try:
            fig, axes = plt.subplots(nrows=9, ncols=2, figsize=(20,27))
            for c_label in range(9):
                ax1, ax2 = axes[c_label,0], axes[c_label,1]
                textA, textB = plotTimeSeriesByClusterLabel(df, LAT, LON, ax1, ax2, cluster_label=c_label, color = 'blue')
                ax1.text(0.1,0.85, textA,  horizontalalignment='left',
                            verticalalignment='center', transform=ax1.transAxes)
