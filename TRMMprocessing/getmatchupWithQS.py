...
for task in taskListInMe:
    ...
    bFname = f'../../downloads/Buoy/extractedGZ/WINDS/T_{lat:02d}{latUnits}_{lon:03d}{lonUnits}_TRMM_matchupNearestFour_2000.nc'
    satFname = glob(f'../../downloads/QS_data/T_{lat}{latUnits}_{lon}{lonUnits}_QS_2000_*.nc')[0]
    writeFname = f'../../downloads/Buoy/extractedGZ/WINDS/T_{lat:02d}{latUnits}_{lon:03d}{lonUnits}_QSandTRMM_2000.nc'
    if os.path.isfile(bFname):
        ...
        buoyWspd = np.array(buoyDS.variables['U10N_TAO'])
        ...
        buoyWdir = np.array(buoyDS.variables['U10N_dir_TAO'])

        buoyWspd2 = np.array(buoyDS.variables['U10N_TAO2'])
        buoyWdir2 = np.array(buoyDS.variables['U10N_dir_TAO2'])

        buoyUwnd2 = np.array(buoyDS.variables['U10N_x_TAO2'])
        buoyVwnd2 = np.array(buoyDS.variables['U10N_y_TAO2'])

        buoySST = np.array(buoyDS.variables['SST_TAO'])
        buoyRH = np.array(buoyDS.variables['RH_TAO'])
        buoyAIRT = np.array(buoyDS.variables['AIRT_TAO'])
        twoHrVarWspd = np.array(buoyDS.variables['wspdVar2hr_TAO'])
        twoHrVarWdir = np.array(buoyDS.variables['wdirVar2hr_TAO'])

        nearSurfRain = np.array(buoyDS.variables['nearSurfRain'][:,:])
        ...
        nearSurfRain = nearSurfRain[:,~mask]
        ...
        if nmatchedData > 0 :
            ...
            wds_nearSurfRain = wds.createVariable('nearSurfRain_TRMM', float, ('N','time',))
            wds_nearSurfRain.units = 'mm/hr'
            wds_nearSurfRain.long_name = 'nearSurfRain variable from TRMM'
            wds_nearSurfRain[:] = nearSurfRain[:,:]
