...
for task in taskListInMe:
    ...
    var = f'../../downloads/TRMM_data/downloaded/T_{lat}{latUnits}_{lon}{lonUnits}_TRMM_*.nc'
    satFname = glob(f'../../downloads/TRMM_data/downloaded/T_{lat}{latUnits}_{lon}{lonUnits}__TRMM_*.nc')[0]
    writeFname = f'../../downloads/Buoy/extractedGZ/WINDS/T_{lat:02d}{latUnits}_{lon:03d}{lonUnits}_TRMM_matchupNearestFour_2000.nc'
    if os.path.isfile(bFname):
        buoyDS = Dataset(bFname)
        satDS = Dataset(satFname)

        ### reading data
        satRain = np.array(satDS.variables['nearSurfRain'])
        ...
        selSatRain = np.zeros((4, nData), dtype=float) * float('nan')
        ...
        for j in range(nData):
            ...
            mask = abs(satDateTime - curTime) < timedelta(minutes = 30)
            npoints = sum(mask)
            ...
            if npoints > 0:
                ...
                arr = np.stack((dist, selRain, timeDiffInSecs, selLon, selLat), axis=0)
            ...
            for i in range(4):
                if i < npoints:
                    ...
                    selSatRain[i,j] = arr[1,i]
                    selSatWtDiff[i,j] = arr[2,i]
                    selSatWLon[i,j] = arr[3,i]
                    selSatWLat[i,j] = arr[4,i]
                else:
                    selSatRain[i,j] = float('nan')
        ...
        mask = np.isnan(buoyWspd) + np.isnan(selSatRain[0,:])
        mask += abs(buoyWspd) > 100
        mask += abs(selSatRain[0,:]) > 100

        selSatRain = selSatRain[:,~mask]
        ...
        if nmatchedData > 0 :
            ...
            timeUnits = 'microseconds since 1990-01-01 00:00:00'
            ...
            wds_U10N_QS = wds.createVariable('nearSurfRain', float, ('N','time',))
            wds_U10N_QS.units = 'mm/hr'
            wds_U10N_QS[:,:] = selSatRain[:,:]
