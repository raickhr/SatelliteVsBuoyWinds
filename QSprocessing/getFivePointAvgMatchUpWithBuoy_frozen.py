...
METOP_DIR = ''
SAT_DIR = ROOT_DIR / 'QuikSCAT_data'/'fivePointAverageAtBuoyPos'/ METOP_DIR
...
def match_times_withFrozenField(
    ...
    for ii in range(ndata):
        ...
        try:
            while deltaX < path_length_m and deltaT < (600*60) :#i86400:
                left -= 1
                if left < 0:
                    left = 0 
...
def process_matchup(lat, lon):
    ...
    output_filename = f'T_{coord_str}_COARE3p5_1999_2009_2hrMeanVar_QS_Matchup_fz.nc'
    out_file = BUOY_DIR /'fivePointAvg'/ METOP_DIR/ output_filename
