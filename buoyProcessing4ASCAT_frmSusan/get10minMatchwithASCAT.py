...
for task in taskListInMe:
    ...
    bFname = f'../../downloads/SusanDownload/TMA_HR/MET/T_{lat:02d}{latUnits}_{lon:03d}{lonUnits}_COARE3p5_2007.nc'
    satFname = glob(f'../../downloads/ASCAT_data/BuoyLocs/T_{lat}{latUnits}_{lon}{lonUnits}_ASCAT_2007_*.nc')[0]
    writeFname = f'../../downloads/SusanDownload/TMA_HR/MET/T_{lat:02d}{latUnits}_{lon:03d}{lonUnits}_matchupNearestFour_2007.nc'
    ...
    if os.path.isfile(bFname):
        ...
        buoyWspd = np.array(buoyDS.variables['U10N'][:,0,0,0])
        ...
        buoyWdir = np.array(buoyDS.variables['wdir'][:,0,0,0])
        ...
        buoySST = np.array(buoyDS.variables['SST'][:,0,0,0])
        buoyRH = np.array(buoyDS.variables['RH'][:,0,0,0])
        buoyAIRT = np.array(buoyDS.variables['AIRT'][:,0,0,0])
