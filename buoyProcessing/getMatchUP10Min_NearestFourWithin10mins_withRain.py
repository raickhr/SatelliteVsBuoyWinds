...
for task in taskListInMe:
    ...
    bFname = f'../../downloads/Buoy/extractedGZ/WINDS/T_{lat:02d}{latUnits}_{lon:03d}{lonUnits}_withRAIN_COARE3p5_2000.nc'
    satFname = glob(f'../../downloads/QS_data/T_{lat}{latUnits}_{lon}{lonUnits}_QS_2000_*.nc')[0]
    writeFname = f'../../downloads/Buoy/extractedGZ/WINDS/T_{lat:02d}{latUnits}_{lon:03d}{lonUnits}_matchupNearestFour_withRain_2000.nc'
    if os.path.isfile(bFname):
        ...
        buoyRAIN = np.array(buoyDS.variables['RAIN'])
        ...
        if nmatchedData > 0 :
            ...
            wds_RAIN = wds.createVariable('RAIN_TAO', float, ('time',))
            wds_RAIN.units = 'mm/hr'
            wds_RAIN.long_name = 'precipitation'
            wds_RAIN[:] = buoyRAIN[:]
