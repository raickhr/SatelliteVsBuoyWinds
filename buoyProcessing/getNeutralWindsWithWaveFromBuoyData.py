...
def fillNanAndRemove(buoy, timeList, speed, wdir, uwnd, vwnd, airt, rh, sst, waveVTPK, waveVPED, waveVHM0):
    # if (len(speed) + 
    #     len(airt) + 
    #     len(rh) + 
    #     len(sst))/4 != len(airt):
    ...
    mask += np.isnan(rh)
    mask += np.isnan(sst)
    mask += np.isnan(waveVTPK)
    mask += np.isnan(waveVPED)
    mask += np.isnan(waveVHM0)

    timeList = np.array(timeList)
    timeList = timeList[~mask]
    timeList = timeList.tolist()

    speed = speed[~mask] # = float('nan')
    wdir = wdir[~mask]
    uwnd = uwnd[~mask] # = float('nan')
    vwnd = vwnd[~mask] # = float('nan')
    airt = airt[~mask] # = float('nan')
    rh = rh[~mask] # = float('nan')
    sst = sst[~mask] # = float('nan')
    waveVTPK = waveVTPK[~mask] # = float('nan')
    waveVHM0 = waveVHM0[~mask] # = float('nan')
    waveVPED = waveVPED[~mask] # = float('nan')

    return timeList, speed, wdir, uwnd, vwnd, airt, rh, sst, waveVTPK, waveVPED, waveVHM0
...
for task in taskListInMe:
    ...
    waveF = glob(f'../../downloads/WaveReanalysis/all/T_{lat:d}{latUnits}_{lon:d}{lonUnits}_wave_*.nc')[0]
    

    if os.path.isfile(bWinds):
        #print(f'{bWinds} present')
        writeFname = f'../../downloads/Buoy/extractedGZ/WINDS/T_{lat:02d}{latUnits}_{lon:03d}{lonUnits}_COARE3p5_withWave.nc'

        ds_WIND = Dataset(bWinds)
        ds_SST = Dataset(bSST)
        ds_AIRT = Dataset(bAirT)
        ds_RH = Dataset(bRH)
        ds_wave = Dataset(waveF)
        ...
        wind_dateTimeArr = [datetime(dtm.year, dtm.month, dtm.day, dtm.hour, dtm.minute) for dtm in cftimes]
        ...
        timeArr = np.array(cdfTime)
        cftimes=num2date(timeArr, timeUnit)
        rh_dateTimeArr = [datetime(dtm.year, dtm.month, dtm.day, dtm.hour, dtm.minute) for dtm in cftimes]
        ...
        timeArr = np.array(cdfTime)
        cftimes=num2date(timeArr, timeUnit)
        sst_dateTimeArr = [datetime(dtm.year, dtm.month, dtm.day, dtm.hour, dtm.minute) for dtm in cftimes]
        ...
        timeArr = np.array(cdfTime)
        cftimes=num2date(timeArr, timeUnit)
        airt_dateTimeArr = [datetime(dtm.year, dtm.month, dtm.day, dtm.hour, dtm.minute) for dtm in cftimes]
        ...
        print(f'rank = {rank}', 'waveFile', waveF)

        waveTime = np.array(ds_wave.variables['time'])
        waveTimeUnits = ds_wave.variables['time'].units
        cftimes = num2date(waveTime, waveTimeUnits)
        wave_DateTimeArr = np.array([datetime(dtm.year, dtm.month, dtm.day, dtm.hour, dtm.minute) for dtm in cftimes])
        waveFileMask = wave_DateTimeArr > datetime(2007,12,31)
        wave_DateTimeArr = wave_DateTimeArr[~waveFileMask]
        waveLat = np.array(ds_wave.variables['lat'])[~waveFileMask]
        waveLon = np.array(ds_wave.variables['lon'])[~waveFileMask]

        waveVTPK = np.array(ds_wave.variables['VTPK'])[~waveFileMask]
        waveVPED = np.array(ds_wave.variables['VPED'])[~waveFileMask]
        waveVHM0 = np.array(ds_wave.variables['VHM0'])[~waveFileMask]

        # plt.plot(wave_DateTimeArr, waveVTPK)
        # plt.show()

        VTPK_dateTimeArr = wave_DateTimeArr.copy().tolist()
        VTPK_dateTimeArr, waveVTPK = removeNan(VTPK_dateTimeArr, waveVTPK)
        VTPK_dateTimeArr, waveVTPK = removeDuplicateAndSort(VTPK_dateTimeArr, waveVTPK)

        VPED_dateTimeArr = wave_DateTimeArr.copy().tolist()
        VPED_dateTimeArr, waveVPED = removeNan(VPED_dateTimeArr, waveVPED)
        VPED_dateTimeArr, waveVPED = removeDuplicateAndSort(VPED_dateTimeArr, waveVPED)

        VHM0_dateTimeArr = wave_DateTimeArr.copy().tolist()
        VHM0_dateTimeArr, waveVHM0 = removeNan(VHM0_dateTimeArr, waveVHM0)
        VHM0_dateTimeArr, waveVHM0 = removeDuplicateAndSort(VHM0_dateTimeArr, waveVHM0)

        # plt.plot(VTPK_dateTimeArr, waveVTPK)
        # plt.show()


        timeList = wind_dateTimeArr + uwnd_dateTimeArr + vwnd_dateTimeArr + rh_dateTimeArr +sst_dateTimeArr +airt_dateTimeArr + VTPK_dateTimeArr + VPED_dateTimeArr + VHM0_dateTimeArr
        timeList.sort()
        ...
        VTPK_dateTimeArr, timeList, waveVTPK, nanArr = fillData(VTPK_dateTimeArr, timeList, waveVTPK, nanArr)
        VPED_dateTimeArr, timeList, waveVPED, nanArr = fillData(VPED_dateTimeArr, timeList, waveVPED, nanArr)
        VHM0_dateTimeArr, timeList, waveVHM0, nanArr = fillData(VHM0_dateTimeArr, timeList, waveVHM0, nanArr)

        buoy =f'{lat:02d}{latUnits}_{lon:03d}{lonUnits}'
        
        #speed, airt, rh, sst = fillMask(buoy, speed, airt, rh, sst)
        print(f'sst {sst.shape}, VTPK {waveVTPK.shape}, VPED {waveVPED.shape}, VHM0 {waveVHM0.shape}')

        timeList, speed, wdir, uwnd, vwnd, airt, rh, sst, waveVTPK, waveVPED, waveVHM0 = fillNanAndRemove(buoy, timeList, speed, wdir, uwnd, vwnd, airt, rh, sst, waveVTPK, waveVPED, waveVHM0)

        # plt.plot(timeList, speed)
        # plt.plot(timeList, waveVTPK)
        # plt.show()

        # waveDateTimeArr, waveData = selectWithWaveData(waveF, LON, LAT, wspd_dateTimeArr, speed, wdir, uwnd, vwnd, rh, sst, airt)

        # waveDist = waveData[0,:]
        # waveTdiff = waveData[1,:]
        # waveLon = waveData[2,:]
        # waveLat = waveData[3,:]
        # waveVTPK = waveData[4,:]
        # waveVPED = waveData[5,:]
        ...
        try:
            ...
            coareOutPutArr2 = coare35vn(speed, airt, rh, sst, cp = cp, sigH = waveVHM0, zu=4.0, zt = 3, zq = 3)
        except:
            print('ERROR ',bWinds)
        ...
        wds_time[:] = date2num(timeList, timeUnit)
        ...
        print(f'T_{lat:02d}{latUnits}_{lon:03d}{lonUnits} done')
