...
def match_times(ds_matchups, 
    ...
    time_matchups = ds_matchups['AS_TIME'].to_numpy()
    time_wave = ds_wave['WAVE_TIME'].to_numpy()
    time_glorys = ds_glorys['GLORYS_TIME'].to_numpy() + np.timedelta64(12, 'h')

    ds_matchups['fillNaN_wave'] = xr.DataArray(np.zeros(len(time_matchups), dtype=int), dims = ['AS_TIME'])
    ds_matchups['fillNaN_glorys'] = xr.DataArray(np.zeros(len(time_matchups), dtype=int), dims = ['AS_TIME'])
    ...
    ds_wave_sel = ds_wave_sel.rename_dims({'WAVE_TIME': 'AS_TIME'})

    ds_glorys_sel = ds_glorys_sel.drop_vars('GLORYS_TIME')
    ds_glorys_sel = ds_glorys_sel.rename_dims({'GLORYS_TIME': 'AS_TIME'})

    ds_wave_sel['WAVE_TIME'] = xr.DataArray(WAVE_TIME, dims = ['AS_TIME'])
    ds_glorys_sel['GLORYS_TIME'] = xr.DataArray(GLORYS_TIME, dims = ['AS_TIME'])
...
def process_matchup(lat, lon):
    ...
    buoySatMatchup_filename = f'T_{coord_str}_COARE3p5_2013_2020_2hrMeanVar_AS_Matchup.nc'
    buoySatMatchup_file = BUOY_DIR / METOP_DIR/ buoySatMatchup_filename

    output_filename = f'T_{coord_str}_COARE3p5_2013_2020_2hrMeanVar_AS_Buoy_Glorys_Wave_Matchup.nc'
    ...
    try:
        ...
        ds_matchup = ds_matchup.sortby('AS_TIME')
        ds_wave = ds_wave.sortby('time')
        ds_glorys = ds_glorys.sortby('time')

        ds_matchup = convert_to_datetime(ds_matchup, 'AS_TIME')
        ...
        merged.to_netcdf(out_file, unlimited_dims='AS_TIME')
