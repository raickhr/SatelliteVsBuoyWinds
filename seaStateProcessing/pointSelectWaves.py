...
for lat in LAT_LIST:
    ...
    for lon in LON_LIST:
        ...
        fileName = f'T_{abs(lat):03.0f}{lat_unit}_{abs(LON):03.0f}{lon_unit}_waveReanalysis.nc'
        ...
        nds = ds.isel(latitude = lat_indx, longitude = lon_indx).drop_vars(['latitude', 'longitude'])

        nds.to_netcdf(folder + f'T_{abs(lat):03.0f}{lat_unit}_{abs(LON):03.0f}{lon_unit}_waveReanalysis_xarr.nc', 
                      unlimited_dims = 'time')
