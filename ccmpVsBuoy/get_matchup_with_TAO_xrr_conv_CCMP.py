...
METOP_DIR = ''
SAT_DIR = ROOT_DIR / 'CCMP/atBuoyLocs'/ METOP_DIR
...
def match_times(ds_sat, ds_buoy, 
    ...
    ds_sat_sel = ds_sat.isel(CC_TIME=list(idx_sat))
    ...
    ds_buoy_sel = ds_buoy_sel.rename({'TAO_TIME': 'CC_TIME'})
    ds_buoy_sel['CC_TIME'] = ds_sat_sel['CC_TIME'].to_numpy()
    ds_buoy_sel['TAO_TIME'] = xr.DataArray(TAO_TIME, dims=['CC_TIME'])
...
def process_matchup(lat, lon):
    ...
    sat_file = SAT_DIR / f'CCMP_tseries_{coord_str}.nc'
    output_filename = f'T_{coord_str}_COARE3p5_1999_2009_2hrMeanVar_CC_Matchup.nc'
    out_file = ROOT_DIR / 'CCMP/matchups'/ output_filename
